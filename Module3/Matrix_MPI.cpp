...
void multiply(int **M1, int **M2, int **M3, int num_rows);
...
void head(int num_processes)
{
    ...
    //Run multiply on the first n(num_rows) rows in main
...
void node(int process_rank, int num_processes)
{
    //Calculated sizes for num of rows per processer, size of M2 to broadcast and size to scatter, done each node as the values arnt global
    int num_rows = size / num_processes;
    int broadcast_size = (size * size);
    int scatter_gather_size = (size * size) / num_processes;
